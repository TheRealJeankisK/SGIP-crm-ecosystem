# Documentación de Arquitectura y Modelo C4 - INSECOM CRM
...
Muestra cómo el sistema **INSECOM CRM** se sitúa dentro de la organización, interactuando con los usuarios y los sistemas externos.

```mermaid
graph TB
    subgraph Users ["Usuarios del Sistema"]
        Vendedor["Vendedor / Ingeniero en Campo"]
        Admin["Personal de Administración / Call Center"]
    end

    subgraph CoreSystem ["Frontera del Ecosistema"]
        CRM["INSECOM CRM<br>(Sistema de Gestión Comercial Digital)"]
    end

    subgraph ExternalSystems ["Sistemas Externos de INSECOM"]
        ERP["ERP Contable / Administrativo<br>(SAP HANA)"]
        MailServer["Servidor de Correo SMTP<br>(SendGrid)"]
    end

    Vendedor -->|Registra leads y visita proyectos| CRM
    Admin -->|Califica leads y supervisa ofertas| CRM
    CRM -->|Sincroniza cotizaciones aprobadas| ERP
    CRM -->|Envía correos con PDFs a clientes| MailServer
```
...
```mermaid
graph TD
    User([Vendedor / Administrador]) -->|Peticiones HTTP/REST| Gateway["API Gateway<br>(Nginx Container)"]
    
    subgraph "Docker Compose Ecosystem"
        Gateway -->|Enruta '/'| FE["App 1: Portal Web (Frontend)<br>(HTML5 / JS / CSS en Nginx)"]
        Gateway -->|Enruta '/api/v1/leads'| BE["App 2: Leads Service (Backend API)<br>(FastAPI / Python)"]
        
        BE -->|Persiste leads| DB[("Capa de Datos: MySQL DB<br>(Docker Volume)")]
        BE -->|Publica eventos de leads| Queue["Queue Manager: RabbitMQ Broker<br>(Docker Container)"]
...
Detalle de la arquitectura interna de los microservicios **Leads Service** y **Notification Lambda**.

```mermaid
graph TB
    subgraph "Leads Service Components"
...
3. **API Gateway ➔ Leads Service**: HTTP REST interno.
4. **Leads Service ➔ MySQL**: Protocolo nativo de MySQL sobre TCP/IP (Puerto 3306).
5. **Leads Service ➔ Service Bus**: AMQP sobre WebSockets (Puerto 5671/443).
