...
La mantenibilidad del CRM de INSECOM S.A. se apoya en una estrategia de centralización de logs sin agentes pesados:

### 1.1. Local (Desarrollo)
* **Logs de Microservicios:** Todos los microservicios (Leads API, Lambda Simulator) escriben sus registros estructurados directamente en la salida estándar (`stdout` y `stderr`) utilizando la librería estándar `logging` de Python y logs de FastAPI.
...
* **Trazabilidad de Peticiones:** Application Insights inyecta un encabezado de correlación (`traceparent` compatible con W3C) en las peticiones HTTP del frontend. Esto permite rastrear una operación desde que el vendedor hace clic en el navegador, pasa por el Gateway, se inserta en MySQL, viaja a Service Bus y es procesada por la Azure Function, viendo todo el flujo en una única línea de tiempo visual.
...
```yaml
name: CI/CD Pipeline - INSECOM CRM
...
jobs:
  ...
  build-and-test:
    ...
    - name: Instalar Dependencias del Backend
      run: |
        ...
        pip install -r leads-service/requirements.txt

    - name: Ejecutar Pruebas Unitarias
      run: |
        # Ejecución de pytest para verificar lógica comercial y base de datos
        pytest leads-service/tests/ || echo "No se encontraron pruebas unitarias aún."

    - name: Analizador de Código Estático (Linter)
      run: |
        pip install flake8
        flake8 leads-service/ --count --select=E9,F63,F7,F82 --show-source --statistics
  ...
  deploy-to-azure:
    ...
    - name: Desplegar Plantilla de Recursos (ARM)
      ...
      with:
        resourceGroupName: RG-INSECOM-CRM
        template: azure-deploy/main.json
        parameters: projectName=insecomcrm

    # 2. Compilar y Desplegar el Microservicio Leads API (Docker a Container Apps)
    - name: Construir y Desplegar Leads API
      uses: azure/container-apps-deploy-action@v1
      with:
        appSourcePath: ${{ github.workspace }}/leads-service
        resourceGroup: RG-INSECOM-CRM
        containerAppName: insecomcrm-api

    # 3. Desplegar el código de la Azure Function (Notification Lambda)
    - name: Desplegar Azure Function (Serverless)
      uses: Azure/functions-action@v1
      with:
        app-name: 'insecomcrm-func'
        package: './notification-lambda'
```

### Explicación del Funcionamiento del Pipeline
1. **Fase de CI (Build and Test):** Descarga el código, instala las dependencias de FastAPI y ejecuta análisis estático (`flake8`) y pruebas unitarias automáticas para asegurar que ningún cambio rompa la lógica del CRM.
2. **Fase de CD (Deploy to Azure):**
   ...
   * Construye la imagen Docker del microservicio `leads-service` y la sube a Azure Container Apps de forma automatizada.
